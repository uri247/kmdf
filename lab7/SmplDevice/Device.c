...
--*/
{
    ...
    status = WdfDeviceInitRegisterPnpStateChangeCallback(DeviceInit, WdfDevStatePnpRemoved, SmplDeviceEvtPnpStateChangeRemoved, StateNotificationEnterState);
    if(!NT_SUCCESS(status))
    {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, 1234,"WdfDeviceInitRegisterPnpStateChangeCallback failed! 0x%x\n", status);
        return status;
    }
    ...
    {   //
        ...
        deviceContext->pRingBuffer = RingbufferInitialize(4096);
        if(NULL == deviceContext->pRingBuffer)
        {
            DbgPrintEx(DPFLTR_IHVDRIVER_ID, 1234,"SmplDeviceCreateDevice RingbufferInitialize failed! 0x%x\n", status);
...
///////////////////////////////////////////////////////////////////////////////
// Plug'n'Play callback entering removed state.
///////////////////////////////////////////////////////////////////////////////
VOID
SmplDeviceEvtPnpStateChangeRemoved (
    WDFDEVICE Device,
    PCWDF_DEVICE_PNP_NOTIFICATION_DATA  NotificationData
    )
{
    PDEVICE_CONTEXT pSmplDeviceContext = DeviceGetContext(Device);
    DbgPrintEx(DPFLTR_IHVDRIVER_ID, 1234,"SmplDeviceEvtPnpStateChangeRemoved >>\n");

    if(NULL != pSmplDeviceContext->pRingBuffer)
        RingbufferFree(pSmplDeviceContext->pRingBuffer);

    pSmplDeviceContext->pRingBuffer = NULL;

} // end SmplDeviceEvtPnpStateChangeRemoved
